        ...
        IAccountRepository accountRepository) : Controller
    {
        ...
        public async Task<IActionResult> Deposit(DepositViewModel depositViewModel)
        {
            if (depositViewModel.Amount > 0)
            {
                try
                {
                    ...
                    if (stringId == null)
                    {
                        throw new InvalidIdException("User identifier is null.");
                    }

                    User receiverAccount = await userService.GetUserByStringIdAsync(stringId, q => q.Include(u => u.Account));
                    if (receiverAccount?.Account?.IBAN == null)
                    {
                        throw new AccountNotFountException($"Could not find account for user {receiverAccount?.Email ?? "unknown"}.");
                    }

                    depositViewModel.ReceiverIBAN = receiverAccount.Account.IBAN;
        ...
        public async Task<IActionResult> Index()
        {
            ...
            try
            {
                ...
                AccountViewModel accountViewModel = user.AccountId != null
                    ? await accountService.GetAccountViewModelByIdAsync((int)user.AccountId)
